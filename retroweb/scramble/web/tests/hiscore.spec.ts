...
const SAMPLE = Array.from({ length: 0x21 }, (_, i) => (i + 1) & 0xff);
const SENTINEL = Array(0x21).fill(0x99);
...
test.describe("HIGH SCORE persist", () => {
  ...
  test("top-10 table RAM is saved and poked back after a reload", async ({ page }) => {
