...
test.describe("keyboard input", () => {
  ...
  test("released state is the factory idle on all three ports", async ({ page }) => {
    ...
    expect(await in2(page)).toBe(0x51);
  });
  ...
  test("ArrowUp / ArrowDown clear IN2 bits 0x10 / 0x40", async ({ page }) => {
    ...
    expect(await in2(page)).toBe(0x51 & ~0x10);
    await page.keyboard.up("ArrowUp");
    await page.keyboard.down("ArrowDown");
    expect(await in2(page)).toBe(0x51 & ~0x40);
  ...
  test("Z / Space fire clears IN0 bit 0x08; X bomb clears bit 0x02", async ({ page }) => {
    await page.keyboard.down("KeyZ");
    expect(await in0(page)).toBe(0xFF & ~0x08);
