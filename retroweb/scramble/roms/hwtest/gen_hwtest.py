#!/usr/bin/env python3
"""Assemble the Scramble board hardware self-test ROM (not Konami Scramble).

Boots like a cabinet self-test (crosshatch, then color bars), then holds a
help screen whose 8×8 font is original — not a dump of c2.5f / c1.5h —
explaining that Konami's ROMs are still under copyright and the user
...
HELP_LINES = [
    ...
    (14, "SCRAMBLE ROMS",      0),
    (17, "THEY ARE STILL",     1),
    (19, "UNDER COPYRIGHT",    1),
    (22, "IF YOU OWN A",       0),
    (24, "KONAMI SCRAMBLE SET", 0),
    (26, "LOAD IT YOURSELF",   1),
...
def assemble_main(s1, s2, s3):
    ...
    # NMI: kick the watchdog, bump the frame counter at $4008.
    a.org(0x0066)
    a.db(0xF5)             # PUSH AF
    a.ld_a_nn(0x7000)      # LD A,(7000)  watchdog
    a.ld_a_nn(0x4008)
    a.db(0x3C)             # INC A
    a.ld_nn_a(0x4008)
    a.db(0xF1)             # POP AF
    ...
    # copy_screen: HL → $4800 (0x400) then DE → objram colours ($5001 step 2).
    a.org(0x0100)
    copy_screen = a.pc
    a.ld_de(0x4800)
    a.ld_bc(0x0400)
    ...
    a.db(0x47)             # LD B,A
    wait_loop = a.pc
    a.ld_a_nn(0x8100)      # IN0 (PPI0 A, A8)
    a.ld_nn_a(0x4010)
    a.ld_a_nn(0x7000)      # kick
    a.ld_a_nn(0x4008)
    a.db(0xB8)             # CP B
    ...
    a.ld_nn_a(0x4008)
    a.ld_a(1)
    a.ld_nn_a(0x6801)      # NMI enable
    a.ld_nn_a(0x6803)      # scramble background
    a.ld_nn_a(0x6804)      # Galaxian stars
...
def assemble_sound():
    ...
    def ay1_out(reg, val):
        a.ld_a(reg)
        a.db(0xD3, 0x40)   # OUT (40),A  AY1 address (bit 6)
        a.ld_a(val)
        a.db(0xD3, 0x80)   # OUT (80),A  AY1 data (bit 7)

    def ay2_out(reg, val):
        a.ld_a(reg)
        a.db(0xD3, 0x10)   # OUT (10),A  AY2 address (bit 4)
        a.ld_a(val)
        a.db(0xD3, 0x20)   # OUT (20),A  AY2 data (bit 5)

    a.org(0x0038)
    a.db(0xF5)
    a.ld_a(14)
    a.db(0xD3, 0x40)
    a.db(0xDB, 0x80)       # IN A,(80)   command (bit 7 = AY1 data)
    a.db(0xA7)             # AND A
    jr_z = a.pc
    a.db(0x28, 0)          # JR Z → silence
    ay1_out(0, 0xFE)       # tone A period ~219 Hz
    ay1_out(1, 1)
    ay1_out(7, 0x38)       # tone A on, noise off
    ay1_out(8, 0x0F)
    ay2_out(0, 0xC0)       # AY2 a fifth above
    ay2_out(1, 1)
    ay2_out(7, 0x38)
    ay2_out(8, 0x0C)
    jr_done = a.pc
...
def color_prom():
    # Galaxian PROM: bits 0–2 R, 3–5 G, 6–7 B (Scramble wires both blue bits).
    p = bytearray(32)
    def rgb(r, g, b):
...
def write_c_header(path, blobs):
    ...
    with open(path, "w") as f:
        ...
        f.write("#pragma once\n#include <cstdint>\n#include <array>\n")
        f.write("namespace scramble { namespace hwtest {\n")
...
def main():
    ...
    for k, fname in names.items():
        with open(os.path.join(args.dir, fname), "wb") as f:
            f.write(blobs[k])
    # Loose-chip names a user zip might also use (MAME scramble).
    for i, name in enumerate(("s1.2d", "s2.2e", "s3.2f", "s4.2h",
                              "s5.2j", "s6.2l", "s7.2m", "s8.2p")):
        with open(os.path.join(args.dir, name), "wb") as f:
            f.write(blobs["program"][i * 0x800:(i + 1) * 0x800])
    for i, name in enumerate(("ot1.5c", "ot2.5d", "ot3.5e")):
        with open(os.path.join(args.dir, name), "wb") as f:
            f.write(blobs["sound"][i * 0x800:(i + 1) * 0x800])
    with open(os.path.join(args.dir, "c2.5f"), "wb") as f:
        f.write(blobs["gfx"][:0x800])
    with open(os.path.join(args.dir, "c1.5h"), "wb") as f:
        f.write(blobs["gfx"][0x800:])
    if args.header:
