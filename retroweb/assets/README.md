...
- **`pc486-cpu.png`** — a 286×128 illustration of an Intel 80486DX2 ceramic
  PGA chip (package outline, pin-1 dot, perimeter pins, an homage die
  marking reading "intel486 DX2-66" -- an original drawing, not a trace of
  Intel's actual trademarked logo artwork, the same rule the front panel's
  `.cpu-badge` SVG already follows). Chosen over a chassis screenshot
  because this machine isn't a recreation of a specific historical case the
  way `ibmpcat-panel.jpg` is of a real 5170 -- the chip is what's
  period-accurate here, the "gamer's dream" tower fascia around it isn't.
  Source SVG generated by hand, rasterized with headless Chrome
  (`--screenshot --window-size=286,128` against the `file://` SVG path).
