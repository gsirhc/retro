...
- **`pacman-cabinet.png`** — original artwork approximating a 1980 upright
  cabinet marquee's *layout* (black housing, backlit yellow plexi, the
  game's name in this project's own 8×8 font, a pellet trail, four color
  chips). Not a photo, scan, or redraw of Namco/Midway art: no official
  logotype, no character sprites, no Midway mark. Generated by
  `retroweb/pacman/roms/hwtest/gen_hwtest.py`'s `marquee_png()`
...
- **`frogger-cabinet.png`** — original artwork approximating a 1981 upright
  cabinet marquee's *layout* (black housing, backlit green plexi, the
  game's name in this project's own 8×8 font, five lily-pad circles). Not
  Konami/Sega art. Generated by
  `retroweb/frogger/roms/hwtest/gen_hwtest.py`'s `marquee_png()`
  (`make -C retroweb/frogger/web roms` writes this file). 286×128.
