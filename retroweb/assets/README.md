...
- **`pacman-cabinet.png`** / **`mspacman-cabinet.png`** /
  **`frogger-cabinet.png`** — original landing-page tiles: a generated
  starfield (nebula + planet, tinted per game) with **Courier New Bold**
  titles (a generic system typeface — not a cabinet logotype, character
  sprite, or manufacturer mark). Generated by
  `retroweb/shared/marquee.py` from a raster atlas of that face
  (`courier_new_bold.atlas`; rebuild with
  `python3 retroweb/shared/marquee.py --rebuild-atlas`). 286×128, same
  box as `altair-panel.jpg`.
