...
//
// This is the PRIMARY IDE channel (0x1F0-0x1F7/0x3F6/IRQ14), the only one
// the AT had. This machine also has a secondary channel on the standard
// 0x170-0x177/0x376/IRQ15 addresses carrying the ATAPI CD-ROM (see
// atapi_cdrom.h) -- a separate cable and controller with the same register
// layout, exactly how a 1993-94 board put the HDD and the CD-ROM on
// different channels so a slow 2x CD-ROM couldn't stall disk I/O by
// holding the shared cable.
//
...
public:
    ...
    // finish_read_or_write() stays out of line: it only runs when a paced
    // transfer's wait has actually elapsed.
...
private:
    ...
    // boot-sector-read bug this fixed in PC486_REVIEW.md, where the
