...
// (see PC486_REVIEW.md; a Web Audio renderer is Phase 7's job). What it
...
public:
    ...
    // Inline: the chipset recomputes this AND gate after every single CPU
    // instruction, and on a silent machine every one of those calls is a
    // compare against the current level that changes nothing. See
    // PC486_REVIEW.md §8.
