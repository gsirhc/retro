...
void Pic8259::out_impl(uint16_t port, uint8_t v) {
    if (port == base_) {
        if (v & 0x10) {  // ICW1
            irr_ = isr_ = 0;
            icw_step_ = 1;
...
void Pic8259::raise(int irq) { irr_ = uint8_t(irr_ | (1 << irq)); refresh_pending(); }
void Pic8259::lower(int irq) { irr_ = uint8_t(irr_ & ~(1 << irq)); refresh_pending(); }
