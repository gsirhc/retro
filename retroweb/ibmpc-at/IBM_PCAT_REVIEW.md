...
**Setup required before any of this actually functions**: `GDRIVE_CLIENT_ID` in `app.js` is a placeholder (`REPLACE_ME.apps.googleusercontent.com`) -- `gdriveIsConfigured()` checks for exactly that prefix and reports a clear "Not set up yet" status (verified live and by `tests/gdrive.spec.ts`) rather than letting a bad Client ID reach Google as a cryptic error. To actually turn this on:

1. In Google Cloud Console, create (or reuse) a project, then enable the **Google Drive API** for it.
2. Configure the **OAuth consent screen** (External user type is fine for personal use -- Google's "Testing" publish status supports up to 100 explicitly-added test users, which is plenty for one person's own browsers/devices; no verification review needed at that scale). `drive.file` is one of Google's "sensitive" scopes, so a consent screen using it past Testing/100-user scale would need Google's verification process -- not a concern for personal use, worth knowing if this ever needs to scale beyond that.
3. Create an **OAuth 2.0 Client ID** credential, application type **Web application**. Add every origin this page is actually served from to **Authorized JavaScript origins** -- at minimum the production domain and `http://localhost:8000` (the local preview server, see `retroweb/README.md`); add the Playwright dev-server port (8300) too if a future test suite ever needs a real, non-placeholder Client ID (none does today -- see `tests/gdrive.spec.ts`'s own comment on why).
4. Paste the generated Client ID into `GDRIVE_CLIENT_ID` in `app.js`, replacing the placeholder.

**Tests** (`tests/gdrive.spec.ts`, new): only what's deterministic without a real Google account or network access to Google's own servers -- neither exists in CI, and shouldn't (this suite doesn't fake Google's APIs, it just never calls them). Covers the button's label/enabled state machine (disabled until firmware loads, then "Connect Google Drive…"), that it stays enabled while powered off (a sync uploads *current* state, unlike Reset/Mount-blank which only affect the next power-on), and -- the actually meaningful case given the placeholder Client ID this ships with -- that clicking it fails with a clear "not set up yet" status rather than hanging on "Syncing…" or throwing an uncaught error. Full connect/upload/download behavior against a real Google account is a manual verification step once a real Client ID is configured, not something this suite can or should attempt.
