...
- **Power-on, in a browser that's connected** (`localStorage`'s `retro8080.gdriveConnected` flag, set the first time a sync actually succeeds): pulls the Drive copy down *first* and boots from that instead of the local IndexedDB copy -- this is the actual fix for the reported problem, not just the upload side. Silent, with a short timeout (`gdriveWaitForGis()`/the token request); any failure at all (offline, consent needs to be interactive again, nothing synced yet) falls back to the local/factory image exactly as if Drive were never connected, so a visitor is never stuck looking at a blank page over a cloud hiccup.
