...
test.describe("ROM set loader", () => {
  ...
  test("a rejected upload opens a themed dialog with load instructions", async ({ page }) => {
    await page.locator("#romFile").setInputFiles({
      ...
      buffer: Buffer.from("this is not a Frogger ROM set"),
    });
  ...
  test("tells the visitor which sets are allowed", async ({ page }) => {
    ...
    await expect(legal).toContainText(/frogger/);
  ...
  test("MAME chip names with the right sizes are accepted", async ({ page }) => {
    await page.locator("#romFile").setInputFiles([
      buf("roms/frogger.26", "frogger.26"),
      buf("roms/frogger.27", "frogger.27"),
      buf("roms/frsm3.7", "frsm3.7"),
      buf("roms/frogger.608", "frogger.608"),
      buf("roms/frogger.609", "frogger.609"),
      buf("roms/frogger.610", "frogger.610"),
      buf("roms/frogger.606", "frogger.606"),
      buf("roms/frogger.607", "frogger.607"),
      buf("roms/pr-91.6l", "pr-91.6l"),
    ]);
    await expect(page.locator("#romStatus")).toHaveText("Hardware test ROM");
  });
