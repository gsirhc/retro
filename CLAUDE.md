...
This repo holds recreations of vintage machines (`Emulator8080` — MITS Altair
8800 / Intel 8080; plus `cpu6502`, `commodore64`, `cc65`, `kicad`). The whole
point is **fidelity to the real hardware**. When you work here, that comes first.
...
- `Emulator8080`: `make -C Emulator8080 test` (GoogleTest), `make -C
  Emulator8080/web` (wasm via emscripten).
- Front-end lives in `Emulator8080/web/`; `retro8080.js`/`.wasm` and
  `roms/*.bin` (the generated ones) are regenerated by CI, not committed.
- **Web integration suite:** `make -C Emulator8080/web test` (Playwright; needs
  Node + `npx playwright install chromium`). Drives the real UI in a browser and
  asserts control behaviour + terminal output.
- **Coverage:** `make -C Emulator8080/web coverage` (app.js, V8/monocart) and
  `make -C Emulator8080 coverage` (i8080/disk88/cassette, llvm-cov). Neither
  gates CI. Use `/* v8 ignore next */` only for genuinely unreachable defensive
  paths, never to hide an untested feature.
- **Every control, device, terminal behaviour, and preset has an automated
  test.** `Emulator8080/web/tests/` (Playwright) covers the front end;
  `Emulator8080/tests/` (GoogleTest) covers the core. A change that adds or
  alters behaviour adds or updates a test in the same commit. CI runs both
