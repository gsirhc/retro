...
- **Landing-page card**: add a `.machine-card` `<a>` in `retroweb/index.html`
  (286×128 image in `retroweb/assets/`, name under the shot) — copy an
  existing one's markup exactly, just swap the `href`, image, and name.
  Arcade tiles are generated by `retroweb/shared/marquee.py` (Courier New
  on a starfield), not cabinet scans; the recipe is in
  `retroweb/assets/README.md`.
...
- **Each machine's own `web/Makefile`** should default its *own* `make serve`
  ...
  *fixed* port for its own Playwright suite instead (`8100` altair8800,
  `8200` assembler6502, `8300`/`8310` ibmpc-at, `8500`/`8510` pacman,
  `8600`/`8610` frogger, `8700`/`8710` scramble, `8800`/`8810` galaxian,
  `8900`/`8910` galaga)
...
- **IBM PC/AT** (`retroweb/ibmpc-at/`): `make -C retroweb/ibmpc-at check`
  ...
  `npm ci` + Chromium).
- **Pac-Man Arcade** (`retroweb/pacman/`): `make -C retroweb/pacman check`
  (GoogleTest over the tilemap/sprite video + Namco WSG + machine, including
  `Smoke.HwtestBootsSignatureWatchdogAndPaints`), `make -C
  retroweb/pacman/web wasm roms` (the generated hardware self-test ROM — a
  from-scratch assembler program, not Namco's — is built the same step),
  `make -C retroweb/pacman/web test-install test` (Playwright, including
...
- **Frogger Arcade** (`retroweb/frogger/`): `make -C retroweb/frogger check`
  ...
  `make -C retroweb/frogger/web test-install test` (Playwright). Ships a
  from-scratch self-test ROM; a real Konami/Sega `frogger` set is opt-in
  and browser-local — see `FROGGER_REVIEW.md`.
- **Scramble Arcade** (`retroweb/scramble/`): `make -C retroweb/scramble check`
  (GoogleTest over the dual Z80 + Galaxian video + two AY-3-8910s + PAL 6J,
  including the board smoke),
  `make -C retroweb/scramble/web wasm roms`, `make -C retroweb/scramble/web
  test-install test` (Playwright). Ships a from-scratch self-test ROM; a
  real Konami `scramble` set is opt-in and browser-local — see
  `SCRAMBLE_REVIEW.md`. Shared chips live in `retroweb/shared/galaxian/`.
- **Galaxian Arcade** (`retroweb/galaxian/`): `make -C retroweb/galaxian check`
  (GoogleTest over the single Z80 + Galaxian video + discrete sound, including
  the board smoke), `make -C retroweb/galaxian/web wasm roms`, `make -C
  retroweb/galaxian/web test-install test` (Playwright). Ships a from-scratch
  self-test ROM; a real Namco `galaxian` set is opt-in and browser-local —
  see `GALAXIAN_REVIEW.md`.
- **Galaga Arcade** (`retroweb/galaga/`): `make -C retroweb/galaga check`
  (GoogleTest over three Z80s + Namco video + the shared wavetable sound
  chip + MB8843/MB8844, including the board smoke), `make -C
  retroweb/galaga/web wasm roms`, `make -C retroweb/galaga/web test-install
...
- Front ends live in each machine's own `web/`; generated wasm/ROM/media
  ...
  `scramble.js`/`.wasm`, `galaxian.js`/`.wasm`, `galaga.js`/`.wasm`, disk/tape
  images) are regenerated by CI, not committed — see each machine's `.gitignore`
  entries.
