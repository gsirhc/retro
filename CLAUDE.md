...
- **CI job naming**: `.github/workflows/deploy-emulator.yml` gives every
  ...
  (plus `z80-test` for the shared CPU). `build`
...
- **Pac-Man Arcade** (`retroweb/pacman/`): `make -C retroweb/pacman check`
  (GoogleTest over the Z80 core + tilemap/sprite video + Namco WSG +
  machine), `make -C retroweb/pacman/web wasm roms` (the generated hardware
  self-test ROM — a from-scratch assembler program, not Namco's — is
  built the same step), `make -C retroweb/pacman/web test-install test`
  (Playwright). Ships only that self-test ROM; a real Midway `pacman` ROM
...
- **Frogger Arcade** (`retroweb/frogger/`): `make -C retroweb/frogger check`
  ...
  `make -C retroweb/frogger/web wasm roms`, `make -C retroweb/frogger/web
  test-install test` (Playwright). Ships a from-scratch self-test ROM; a
  real Konami/Sega `frogger` set is opt-in and browser-local — see
  `FROGGER_REVIEW.md`.
- Front ends live in each machine's own `web/`; generated wasm/ROM/media
  files (`retro8080.js`/`.wasm`, `cgoac6502.js`/`.wasm`/`roms/*.bin`,
  `ibmpcat.js`/`.wasm`, `pacman.js`/`.wasm`, `frogger.js`/`.wasm`, disk/tape images) are
